-- ══════════════════════════════════════════════════════════
-- Seed: detail-modal content for the last 2 items (series 3).
-- Run in SQL Editor AFTER 008_seed_mandheling_detail.sql.
-- Idempotent (re-runnable).
--
-- Also renames:
--   "아라비카 케린치 내추럴" → "아라비카 끄린찌 내추럴"
--   "로부스타 테망궁 워시드" → "로부스타 뜨망궁 워시드"
--
-- Photo columns (image / detail_img / detail_img2) are NOT touched —
-- the admin already uploaded all three photo slots per item.
-- Robusta taste/fragrance/body left empty (TBC) — empty fields hide
-- automatically in the modal spec table.
-- ══════════════════════════════════════════════════════════
...
set
  ...
  moisture       = '13% - 14%',
  body           = 4
where name_ko in ('아라비카 케린치 내추럴', '아라비카 끄린찌 내추럴');

-- ── 2) Robusta Temanggung Washed (taste/fragrance/body TBC) ──
update product_items
set
  ...
  moisture       = '13% - 14%',
  body           = 0
where name_ko in ('로부스타 테망궁 워시드', '로부스타 뜨망궁 워시드');

-- ── Verify ──────────────────────────────────────────────────
-- select name_ko, subtitle_ko, region, process, body
--   from product_items
--   where name_ko in ('아라비카 끄린찌 내추럴', '로부스타 뜨망궁 워시드');
